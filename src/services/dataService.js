...
export async function getHistoricoLeituras({ canteiroId, inicio, fim, page = 1, pageSize = 20 } = {}) {
  ...
  // Guarda de regressão (risco #10): page/pageSize inválidos (0, negativo,
  // não-numérico) não podem corromper start/totalPages silenciosamente.
  const pageSizeSeguro = Number.isInteger(pageSize) && pageSize > 0 ? pageSize : 20;
  const pageSeguro = Number.isInteger(page) && page > 0 ? page : 1;
  ...
  const start = (pageSeguro - 1) * pageSizeSeguro;

  const items = lista.slice(start, start + pageSizeSeguro);

  return {
    items,
    total,
    page: pageSeguro,
    pageSize: pageSizeSeguro,
    totalPages: Math.ceil(total / pageSizeSeguro) || 1,
  };
